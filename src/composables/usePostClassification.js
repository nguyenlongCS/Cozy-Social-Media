/*
src/composables/usePostClassification.js
Hệ thống phân loại bài viết tự động bằng Keyword Matching
Logic:
- Dictionary Keywords cho 19 danh mục được định sẵn
- Thuật toán keyword matching với scoring system
- Hỗ trợ đa ngôn ngữ (Tiếng Việt và English)
- Tách biệt hoàn toàn khỏi hệ thống hiện tại
- Chỉ thêm field Tags vào posts collection
...
export function usePostClassification() {
  ...
  // Dictionary Keywords với trọng số
  ...
  const classifyPost = (caption) => {
    ...
    try {
      ...
      console.error('Error in post classification:', error)
    ...
    for (const post of posts) {
      if (post.Caption) {
        ...
        results.push({
          ...
          tags: tags.map(t => t.tag), // Chỉ lấy tên tag
  ...
  const validateClassification = (caption, expectedTags) => {
    const result = classifyPost(caption)
    const predictedTags = result.map(r => r.tag)
    
    const accuracy = expectedTags.filter(tag => 
      predictedTags.includes(tag)
    ).length / Math.max(expectedTags.length, 1)
  ...
  const runTestCases = () => {
    const testCases = [
      {
        caption: "Messi ghi bàn thắng đẹp trong trận đấu hôm nay",
        expected: ["Thể thao"]
      },
      {
        caption: "iPhone 15 có camera tuyệt vời và hiệu năng mạnh mẽ",
        expected: ["Công nghệ"]
      },
      {
        caption: "Món phở bò ngon tuyệt vời tại nhà hàng này",
        expected: ["Ăn uống"]
      },
      {
        caption: "Avengers Endgame là bộ phim Marvel hay nhất",
        expected: ["Phim ảnh"]
      }
    ]

    const results = testCases.map(testCase => 
      validateClassification(testCase.caption, testCase.expected)
    )
