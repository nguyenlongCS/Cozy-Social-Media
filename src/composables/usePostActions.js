...
Quản lý các hành động trên post: xóa, ẩn, tải xuống, báo cáo
...
export function usePostActions() {
  ...
  const checkPostOwnership = (post, currentUserId) => {
    ...
    if (post.UserID !== currentUserId) throw new Error('PERMISSION_DENIED')
    return true
  }

  // Delete post completely (post + comments + likes)
  const deletePost = async (post, currentUserId) => {
    if (!post?.PostID) throw new Error('INVALID_POST')
